...
const baseProperties=raw.map((p,i)=>({id:i+1,slug:p[0],title:p[1],city:p[2],region:p[3],price:p[4],type:p[5],beds:p[6],baths:p[7],area:p[8],image:img(p[9]),agency:agencies[p[10]],features:p[11].split(','),ref:`PIP-${p[3].slice(0,3).toUpperCase()}-${23847+i}`,status:i===6?'Under Offer':'Live',featured:i<7,description:'Designed for effortless indoor-outdoor living, this beautifully considered home combines generous natural light, refined materials and a privileged Portuguese setting. Thoughtful proportions create quiet spaces for daily life and generous rooms for gathering.'}));
const slugify=s=>s.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9]+/g,'-').replace(/(^-|-$)/g,'');
const regionFor=loc=>{const l=loc.toLowerCase();if(/algarve|faro|lagos|albufeira|loule|loulé|portimao|portimão|silves/.test(l))return 'Algarve';if(/lisbon|lisboa|cascais|estoril|sintra|oeiras/.test(l))return l.includes('cascais')||l.includes('estoril')?'Cascais':'Lisbon';if(/porto|braga|aveiro|vila nova|gaia|douro/.test(l))return 'Porto';if(/madeira|funchal|calheta|ponta delgada/.test(l))return 'Madeira';return 'Silver Coast'};
const agencyFor=(name,i)=>{const clean=name||'Independent agency';const mark=clean.split(/\s+/).filter(Boolean).slice(0,2).map(x=>x[0]).join('').toUpperCase();return {slug:`scraped-${slugify(clean)}`,name:clean,mark:mark||'IP',location:'Portugal',count:1,phone:'',email:'',about:'Source agency listing imported from Properties in Portugal.'}};
const scrapedProperties=(scraped||[]).map((p,i)=>{const region=regionFor(p.location||p.name);const bits=(p.name||'Property in Portugal').split(' in ');const type=/villa/i.test(p.name)?'Villa':/apartment|flat/i.test(p.name)?'Apartment':/land|plot/i.test(p.name)?'Land':/townhouse/i.test(p.name)?'Townhouse':'House';const n=parseInt(p.price?.replace(/[^0-9]/g,''),10)||0;return {id:100+i,slug:`${slugify(bits[0]||'property')}-${100+i}`,title:p.name||'Property in Portugal',city:(p.location||'Portugal').split(',')[0].trim(),region,price:n,type,beds:parseInt(p.beds,10)||0,baths:parseInt(p.baths,10)||0,area:parseInt(p.area,10)||0,image:(p.image||'').startsWith('http')?p.image:`https://www.propertiesinportugal.com${p.image||'/img/preload.png'}`,agency:agencyFor(p.agency,i),features:['Source listing','Imported catalogue'],ref:p.ref||`SCR-${100+i}`,status:'Live',featured:false,source:'scraped',sourceUrl:p.sourceUrl,description:p.description||'Listing imported from the live Properties in Portugal catalogue.'}});
